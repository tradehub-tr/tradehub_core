"""Satıcı sidebar'ına "Doğrulamalarım" (Profil & Finans) navigasyon item'ı ekle.

TH Module Registry'de `seller.store.profil.dogrulama` (route /my-verifications)
kaydını oluşturur (idempotent). KYB Doğrulama'nın hemen altında konumlanır
(order 3; Abonelik order 4'e itilir). Mevcut kayıtlara dokunulmaz; sonda
nested-set lft/rgt rebuild.
...
_KEY = "seller.store.profil.dogrulama"


def execute() -> dict:
	from tradehub_core.setup.module_navigation_spec import get_all_modules

	# Abonelik'i order 4'e it (spec'te güncellendi; mevcut DB kaydını da hizala).
	if frappe.db.exists("TH Module Registry", "seller.store.profil.abonelik"):
		frappe.db.set_value(
			"TH Module Registry", "seller.store.profil.abonelik", "display_order", 4
		)
	...
	if not spec:
		frappe.log_error(f"{_KEY} spec'te bulunamadı", "v15_8_10_seed_verification_nav")
		return {"created": []}
	...
	try:
		...
		frappe.log_error("nav rebuild_tree failed", "v15_8_10_seed_verification_nav")
