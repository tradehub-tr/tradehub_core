"""Satıcı sidebar'ına "Abonelik" (Profil & Finans) navigasyon item'ı ekle.

TH Module Registry'de `seller.store.profil.abonelik` (route /abonelik) kaydını
oluşturur (idempotent). Mevcut kayıtlara dokunulmaz; sonda nested-set lft/rgt
rebuild.
...
_KEY = "seller.store.profil.abonelik"
...
def execute() -> dict:
	...
	if not spec:
		frappe.log_error(f"{_KEY} spec'te bulunamadı", "v15_7_5_seed_subscription_nav")
		return {"created": []}
	...
	try:
		...
		frappe.log_error("nav rebuild_tree failed", "v15_7_5_seed_subscription_nav")
