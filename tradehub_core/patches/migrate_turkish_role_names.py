...
def execute():
	...
	if migrated:
		frappe.log_error(
			title=_("Role Migration Complete"), message=_("Migrated roles: {0}").format(", ".join(migrated))
		)

	if skipped:
		frappe.log_error(
			title=_("Role Migration Skipped"), message=_("Skipped roles: {0}").format(", ".join(skipped))
		)
...
def _update_table_role(doctype, fieldname, old_name, new_name):
	...
	frappe.db.sql(
		f"UPDATE `{table_name}` SET `{fieldname}` = %s WHERE `{fieldname}` = %s", (new_name, old_name)
	)
