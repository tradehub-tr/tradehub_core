"""Satıcı sidebar'ına "Medya Gezgini" (Vitrin) navigasyon item'ı ekle.

TH Module Registry'de `seller.store.vitrin.medya_gezgini`
(route /my-media-explorer) kaydını oluşturur (idempotent). Medya
Kütüphanesi'nin hemen altında konumlanır (order 2).
...
Satıcı Medya Gezgini view'ı (`/my-media-explorer`, SellerMediaExplorerView)
route ve menü tanımı olarak vardı ama registry kaydı olmadığı için satıcı
sidebar'ında hiç görünmüyordu — Medya Kütüphanesi'nin v15_8_10'daki
...
_KEY = "seller.store.vitrin.medya_gezgini"
...
def execute() -> dict:
	...
	if not spec:
		frappe.log_error(f"{_KEY} spec'te bulunamadı", "v15_9_36_seed_seller_media_explorer_nav")
		return {"created": []}
	...
	try:
		...
		frappe.log_error("nav rebuild_tree failed", "v15_9_36_seed_seller_media_explorer_nav")
