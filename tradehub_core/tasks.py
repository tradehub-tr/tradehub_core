...
def calculate_customer_grades():
	...
	if frappe.cache().get_value(lock_key):
		frappe.log_error("calculate_customer_grades already running", "Scheduler Lock")
		return
...
def _run_customer_grading():
	...
	frappe.logger().info(f"Starting customer grade calculation for {len(buyers)} buyers...")
	...
	frappe.logger().info(f"Customer grade calculation complete. Processed: {processed}, Errors: {errors}")
...
def _update_seller_customer_grades(buyer_name, customer_grade):
	...
	seller_grades = frappe.get_all(
		"Seller Customer Grade", filters={"buyer": buyer_name, "status": "Active"}, fields=["name"]
	)
...
def recalculate_buyer_metrics():
	...
	if frappe.cache().get_value(lock_key):
		frappe.log_error("recalculate_buyer_metrics already running", "Scheduler Lock")
		return
...
def _run_buyer_metrics_recalculation():
	...
	frappe.logger().info(f"Starting buyer metrics recalculation for {len(buyers)} buyers...")
	...
	frappe.logger().info(f"Buyer metrics recalculation complete. Processed: {processed}, Errors: {errors}")
...
def _recalculate_metrics_for_buyer(buyer_name):
	...
	if frappe.db.exists("DocType", "Return Request"):
		return_count = frappe.db.count(
			"Return Request", {"buyer": buyer_name, "status": ["not in", ["Cancelled", "Draft"]]}
		)
	...
	if frappe.db.exists("DocType", "Seller Feedback"):
		feedback_count = frappe.db.count("Seller Feedback", {"buyer": buyer_name})
...
def calculate_buyer_scores():
	...
	if frappe.cache().get_value(lock_key):
		frappe.log_error("calculate_buyer_scores already running", "Scheduler Lock")
		return
...
def _run_buyer_score_calculation():
	...
	frappe.logger().info(f"Starting buyer score calculation for {len(buyers)} buyers...")
	...
	frappe.logger().info(f"Buyer score calculation complete. Processed: {processed}, Errors: {errors}")
...
def buyer_level_tasks():
	...
	if frappe.cache().get_value(lock_key):
		frappe.log_error("buyer_level_tasks already running", "Scheduler Lock")
		return
...
def _run_buyer_level_evaluation():
	...
	levels = frappe.get_all(
		"Buyer Level", filters={"status": "Active"}, fields=["name", "level_rank"], order_by="level_rank desc"
	)
	...
	frappe.logger().info(f"Starting buyer level evaluation for {len(buyers)} buyers...")
	...
	frappe.logger().info(f"Buyer level evaluation complete. Processed: {processed}, Errors: {errors}")
...
def _evaluate_level_for_buyer(buyer, levels):
	...
	if not new_level:
		default_level = frappe.db.get_value("Buyer Level", {"is_default": 1, "status": "Active"}, "name")
...
def update_buyer_kpi_template_stats():
	...
	if frappe.cache().get_value(lock_key):
		frappe.log_error("update_buyer_kpi_template_stats already running", "Scheduler Lock")
		return
...
def _run_buyer_kpi_template_stats_update():
	...
	if frappe.db.exists("DocType", "Buyer KPI Template"):
		templates = frappe.get_all("Buyer KPI Template", filters={"status": "Active"}, fields=["name"])

		frappe.logger().info(f"Updating statistics for {len(templates)} Buyer KPI Templates...")
	...
	if frappe.db.exists("DocType", "Buyer Level"):
		...
		frappe.logger().info(f"Updating buyer counts for {len(levels)} Buyer Levels...")
...
def _update_template_stats(template_name):
	...
	if frappe.db.exists("DocType", "Buyer KPI Score Log"):
		usage_count = frappe.db.count("Buyer KPI Score Log", {"kpi_template": template_name})
...
def refresh_user_segments():
	...
	if frappe.cache().get_value(lock_key):
		frappe.log_error("refresh_user_segments already running", "Scheduler Lock")
		return
...
def _run_user_segment_refresh():
	...
	frappe.logger().info(f"Starting user segment refresh for {len(segments)} segments...")
	...
	frappe.logger().info(f"User segment refresh complete. Processed: {processed}, Errors: {errors}")
...
def _refresh_single_segment(segment):
	...
	active_count = frappe.db.count("Segment Member", {"segment": segment_doc.name, "is_active": 1})
...
def calculate_buyer_kpi_scores():
	...
	if frappe.cache().get_value(lock_key):
		frappe.log_error("calculate_buyer_kpi_scores already running", "Scheduler Lock")
		return
...
def _run_buyer_kpi_score_calculation():
	...
	default_template_name = frappe.db.get_value(
		"Buyer KPI Template", {"is_default": 1, "status": "Active"}, "name"
	)
	...
	frappe.logger().info(f"Starting buyer KPI score calculation for {len(buyers)} buyers...")
...
def _normalize_metric(
	...
	if scoring_method == "Logarithmic":
		...
		if target_value <= 0:
			return 0.0
		return min(100.0, safe_divide(math.log10(raw_value + 1), math.log10(target_value + 1)) * 100.0)
...
def aggregate_buyer_kpi_summaries():
	...
	if frappe.cache().get_value(lock_key):
		frappe.log_error("aggregate_buyer_kpi_summaries already running", "Scheduler Lock")
		return
...
def _run_buyer_kpi_summary_aggregation():
	...
	if frappe.db.exists("DocType", "Buyer KPI Template"):
		...
		frappe.logger().info(f"Updating KPI statistics for {len(templates)} Buyer KPI Templates...")
	...
	if frappe.db.exists("DocType", "User Profile"):
		...
		frappe.logger().info(f"Aggregating KPI summaries for {len(buyers)} buyers...")
...
def _update_kpi_template_statistics(template_name):
	...
	usage_count = frappe.db.count(
		"Buyer KPI Score Log", {"kpi_template": template_name, "status": "Finalized"}
	)
