...
def on_customer_update(doc, method):
	...
	try:
		...
		frappe.logger().info(f"Synced Customer {doc.name} to Buyer Profile {buyer_profile_id}")

	except Exception as e:
		frappe.log_error(f"Failed to sync Customer {doc.name}: {str(e)}", "ERPNext Sync Error")
...
def on_customer_insert(doc, method):
	...
	try:
		...
		if existing:
			...
			frappe.logger().info(f"Linked Customer {doc.name} to existing Buyer Profile {existing}")
			return
		...
		frappe.logger().info(f"Created Buyer Profile {buyer_profile.name} for Customer {doc.name}")

	except Exception as e:
		frappe.log_error(
			f"Failed to create Buyer Profile for Customer {doc.name}: {str(e)}", "ERPNext Sync Error"
		)
...
def on_customer_delete(doc, method):
	...
	try:
		...
		frappe.logger().info(f"Unlinked Buyer Profile {buyer_profile_id} from deleted Customer {doc.name}")
