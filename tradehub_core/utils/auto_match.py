...
def auto_match_columns(source_columns, target_doctype, import_type, tenant=None, seller=None):
	...
	historical_mappings = _get_historical_mappings(target_doctype, import_type, tenant, seller)
	...
	for source_col in source_columns:
		match = _match_single_column(source_col, target_fields, historical_mappings, used_targets)
...
@frappe.whitelist()
def save_to_mapping_history(confirmed_mappings, target_doctype, import_type, tenant=None, seller=None):
	"""
...
def apply_mapping_template(import_job_name, template_name):
	...
	if not frappe.db.exists("Import Job", import_job_name):
		frappe.throw(_("Import Job {0} does not exist").format(import_job_name), frappe.DoesNotExistError)

	# Validate template exists
	if not frappe.db.exists("Mapping Template", template_name):
		frappe.throw(_("Mapping Template {0} does not exist").format(template_name), frappe.DoesNotExistError)
	...
	# Apply to import job
	import_job.column_mapping = json.dumps(column_mapping, indent=2, ensure_ascii=False, default=str)
...
def auto_match_for_import_job(import_job_name):
	...
	if job.status != "Pending":
		frappe.throw(
			_("Auto-match is only available when Import Job status is Pending"), frappe.ValidationError
		)

	if not job.import_file:
		frappe.throw(_("No import file attached to this job"), frappe.ValidationError)
	...
	results = auto_match_columns(
		headers, target_doctype, job.import_type, tenant=job.tenant, seller=job.seller
	)
...
def _match_single_column(source_col, target_fields, historical_mappings, used_targets):
	...
	for field in target_fields:
		...
		if (
			source_normalized == field["label_normalized"]
			or source_normalized == field["fieldname_normalized"]
		):
			return {
				"source_column": source_col,
				"target_fieldname": field["fieldname"],
	...
	for field in target_fields:
		...
		ratio_label = SequenceMatcher(None, source_normalized, field["label_normalized"]).ratio()
		ratio_fieldname = SequenceMatcher(None, source_normalized, field["fieldname_normalized"]).ratio()
...
def _find_existing_history(source_col, target_field, target_doctype, import_type, tenant=None, seller=None):
	"""
