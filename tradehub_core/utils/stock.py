...
def _recalculate_available(listing_name):
    ...
    vals = frappe.db.get_value(
        ...
        ["stock_qty", "reserved_qty", "available_qty",
         "track_inventory", "status", "seller_profile",
         "title", "listing_code", "low_stock_threshold"],
        as_dict=True,
    )
    if not vals:
        return
...
def _send_stock_alert_if_needed(listing_name, vals, old_available, new_available):
    ...
    seller_user = (
        ...
        if vals.seller_profile else None
    )
    if not seller_user:
        return
    ...
    if new_available <= 0 and old_available > 0:
        ...
        alert_message = _("{0} ürününüzün stoğu {1} adede düştü.").format(
            title_text, int(new_available)
        )
    ...
    already_sent = frappe.db.exists("Platform Notification", {
