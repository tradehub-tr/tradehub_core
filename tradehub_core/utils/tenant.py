...
def get_current_tenant():
	...
	if frappe.session.user and frappe.session.user != "Guest":
		user_tenant = frappe.db.get_value("User", frappe.session.user, "default_tenant", cache=True)
		if user_tenant:
			return user_tenant
...
def set_tenant(tenant_name):
	...
	if tenant_doc.get("disabled"):
		frappe.throw(_("Tenant {0} is disabled").format(tenant_name), frappe.ValidationError)
...
def validate_tenant(doc, method=None):
	...
	if doc.get("tenant") and current_tenant:
		if doc.tenant != current_tenant:
			frappe.throw(
				_("You do not have permission to modify documents from tenant {0}").format(doc.tenant),
...
def clear_tenant_cache():
	...
	tenant = get_current_tenant()
	if tenant:
		cache_keys = [f"tenant_config:{tenant}", f"tenant_permissions:{tenant}", f"tenant_users:{tenant}"]
...
def get_tenant_users(tenant_name):
	...
	if users is None:
		users = frappe.get_all("User", filters={"default_tenant": tenant_name, "enabled": 1}, pluck="name")
...
def is_tenant_admin(user=None, tenant=None):
	...
	is_admin = frappe.db.get_value("User", user, "is_tenant_admin", cache=True)
...
def _is_system_doctype(doctype):
	...
	system_doctypes = [
		...
		"Workflow Transition",
		"Tenant",
		"Organization",  # Tenant and Organization manage themselves
	]
