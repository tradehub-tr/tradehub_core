...
def _get_seller_tenant(seller_name: str) -> str | None:
	"""
...
def validate_tenant_seller_match(doc: "frappe.model.document.Document", method: str = None) -> None:
	"""
	...
	if not seller_tenant:
		frappe.throw(
			_(
				"The selected seller ({0}) does not have a tenant assigned. "
	...
	if doc_tenant != seller_tenant:
		frappe.throw(
			_(
				"Tenant and Seller mismatch: The selected seller ({0}) "
				"belongs to tenant '{1}', but this document is assigned to "
				"tenant '{2}'. Please select a seller from the correct tenant "
				"or update the tenant field."
			).format(doc_seller, seller_tenant, doc_tenant),
			frappe.ValidationError,
		)


def auto_set_tenant_from_seller(doc: "frappe.model.document.Document", method: str = None) -> None:
	"""
