...
def _enforce_single_policy(policy: dict) -> None:
	...
	while True:
		...
		for doc_name in expired_names:
			try:
				...
				frappe.log_error(f"Anonymize failed: {policy.ref_doctype}/{doc_name}")
	...
	if total_processed:
		frappe.logger().info(f"Retention: anonymized {total_processed} records in {policy.ref_doctype}")
