...
def _compute_category_scores(user: str) -> dict:
    ...
    if scores:
        ...
        return {
            c: {"score": scores[c], "parent": parent_map.get(c)}
            for c in scores
        }
    return {}
...
def get_tailored_selections(limit: int = 9):
    ...
    # Cache layer
    cache_key = f"tailored:global" if is_guest else f"tailored:user:{user}"
...
@frappe.whitelist(allow_guest=True)
def get_tailored_group_detail(category: str, subcategory: str = None,
    ...
    return {
        ...
        "subCategories": [
            {"slug": s.url_slug or s.name, "name": s.category_name or s.name}
