...
def _scale_keys(prefix: str) -> frozenset:
	...
	return frozenset(
		f"--color-{prefix}-{step}" for step in (50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 950)
	)
		...
		for raw_key, raw_value in data.items():
			...
			for pattern in _FORBIDDEN_VALUE_PATTERNS:
				if pattern in lowered:
					frappe.throw(_("{0} değeri yasak karakter içeriyor: {1}").format(key, pattern))

			if key in _COLOR_KEYS:
				if not _COLOR_VALUE_RE.match(lowered):
					frappe.throw(_("{0} geçerli bir renk değeri değil: {1}").format(key, value))
			elif key in _NUMERIC_KEYS:
				if not _NUMERIC_VALUE_RE.match(lowered):
					frappe.throw(_("{0} geçerli bir sayısal değer değil: {1}").format(key, value))
			else:
				if not _TEXT_VALUE_RE.match(value):
					frappe.throw(_("{0} geçersiz metin değeri içeriyor: {1}").format(key, value))
