...
def credit_seller_for_order(
	...
	balance.add_earnings(
		amount=gross_amount, order_name=sub_order, description=f"Order completed: {sub_order}"
	)
	...
	balance.record_order_completion(order_value=gross_amount, commission_amount=commission_amount)

	# Release to available if requested (immediate payout)
	if release_immediately:
		balance.release_to_available(
			amount=flt(amount), escrow_name=sub_order, description=f"Immediate release for order {sub_order}"
		)
...
def release_escrow_to_balance(seller: str, sub_order: str, amount: float, escrow_name: str = None) -> dict:
	"""
...
def process_order_refund_from_balance(
	seller: str, sub_order: str, refund_amount: float, commission_refund: float = 0
) -> dict:
	"""
	...
	if net_deduction > 0:
		balance.process_refund(
			amount=net_deduction, order_name=sub_order, description=f"Refund processed for order {sub_order}"
		)
...
def calculate_commission(
	seller: str, order_total: float, category: str = None, use_seller_plan: bool = True
) -> dict:
	"""
...
def schedule_escrow_release(seller: str, sub_order: str, amount: float, hold_days: int = None) -> dict:
	"""
	...
	frappe.publish_realtime(
		...
		{"seller": seller, "sub_order": sub_order, "amount": amount, "release_date": str(release_date)},
		doctype="Sub Order",
		docname=sub_order,
	)
...
def process_escrow_release(sub_order: str) -> dict:
	...
	result = release_escrow_to_balance(
		seller=sub_order_doc.seller, sub_order=sub_order, amount=flt(sub_order_doc.seller_payout)
	)
...
def process_pending_escrow_releases():
	...
	for so in sub_orders:
		try:
			...
			frappe.log_error(
				f"Failed to process escrow release for {so.name}: {str(e)}", "Escrow Release Error"
			)

	return {"status": "completed", "processed": processed, "failed": failed, "errors": errors}
...
def on_sub_order_completed(sub_order_name: str) -> dict:
	...
	schedule_result = schedule_escrow_release(
		seller=sub_order.seller, sub_order=sub_order_name, amount=flt(sub_order.seller_payout)
	)

	# Update sub order
	sub_order.db_set("payout_status", "Scheduled")

	return {"credit_result": result, "escrow_schedule": schedule_result}
...
def calculate_order_commission(seller: str, order_total: float, category: str = None) -> dict:
	...
	"""
	return calculate_commission(seller=seller, order_total=flt(order_total), category=category)
