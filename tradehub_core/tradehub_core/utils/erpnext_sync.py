...
def get_sync_settings(tenant: str = None) -> dict[str, Any]:
	...
	settings_name = frappe.db.get_value("ERPNext Integration Settings", {"tenant": tenant, "enabled": 1})
...
def sync_seller_to_supplier(
	seller_name: str, force_update: bool = False, ignore_permissions: bool = False
) -> str | None:
	"""
	...
	if not is_erpnext_installed():
		frappe.throw(_("ERPNext is not installed. Cannot sync to Supplier."), frappe.AppNotInstalledError)
	...
	if not seller.sync_with_erpnext:
		frappe.msgprint(_("ERPNext sync is not enabled for this seller"), indicator="yellow")
		return None
	...
	try:
		...
		if seller.tax_id:
			existing_supplier = frappe.db.get_value("Supplier", {"tax_id": seller.tax_id}, "name")

		if not existing_supplier and seller.company_name:
			existing_supplier = frappe.db.get_value(
				"Supplier", {"supplier_name": seller.company_name}, "name"
			)
		...
		_log_sync_event(
			...
			"Supplier",
			supplier_name,
			"create" if not existing_supplier else "update",
		)
		...
		# Log error
		frappe.log_error(message=str(e), title=f"ERPNext Sync Error: Seller {seller_name}")
		raise
...
def sync_supplier_to_seller(
	supplier_name: str, seller_name: str = None, ignore_permissions: bool = False
) -> str | None:
	"""
	...
	try:
		...
		frappe.log_error(message=str(e), title=f"ERPNext Reverse Sync Error: Supplier {supplier_name}")
		return None
...
def sync_buyer_to_customer(
	buyer_name: str, force_update: bool = False, ignore_permissions: bool = False
) -> str | None:
	"""
	...
	# Check ERPNext availability
	if not is_erpnext_installed():
		frappe.throw(_("ERPNext is not installed. Cannot sync to Customer."), frappe.AppNotInstalledError)
	...
	if not buyer.get("sync_with_erpnext"):
		frappe.msgprint(_("ERPNext sync is not enabled for this buyer"), indicator="yellow")
		return None
	...
	try:
		...
		if buyer.get("tax_id"):
			existing_customer = frappe.db.get_value("Customer", {"tax_id": buyer.tax_id}, "name")

		if not existing_customer and buyer.company_name:
			existing_customer = frappe.db.get_value("Customer", {"customer_name": buyer.company_name}, "name")
		...
		_log_sync_event(
			...
			"Customer",
			customer_name,
			"create" if not existing_customer else "update",
		)

		return customer_name

	except Exception as e:
		frappe.log_error(message=str(e), title=f"ERPNext Sync Error: Buyer {buyer_name}")
		raise
...
def sync_customer_to_buyer(
	customer_name: str, buyer_name: str = None, ignore_permissions: bool = False
) -> str | None:
	"""
	...
	try:
		...
		frappe.log_error(message=str(e), title=f"ERPNext Reverse Sync Error: Customer {customer_name}")
		return None
		...
		if not target.item_group:
			...
			if source.category:
				...
				item_group = frappe.db.get_value(
					"Item Group", {"item_group_name": source.category_name}, "name"
				)
				if item_group:
...
def sync_sku_to_item(
	sku_product: str, force_update: bool = False, ignore_permissions: bool = False
) -> str | None:
	"""
	...
	# Check ERPNext availability
	if not is_erpnext_installed():
		frappe.throw(_("ERPNext is not installed. Cannot sync to Item."), frappe.AppNotInstalledError)

	if not check_erpnext_doctype("Item"):
		frappe.throw(
			_("Item DocType not found. Please ensure ERPNext is properly configured."), frappe.ValidationError
		)

	# Get SKU product document
	if not frappe.db.exists("SKU Product", sku_product):
		frappe.throw(_("SKU Product {0} not found").format(sku_product), frappe.DoesNotExistError)

	sku = frappe.get_doc("SKU Product", sku_product)

	# Check if sync is enabled
	if not sku.sync_with_erpnext:
		frappe.msgprint(_("ERPNext sync is not enabled for this SKU Product"), indicator="yellow")
		return None
	...
	try:
		...
		existing_item = frappe.db.get_value("Item", {"item_code": sku.sku_code}, "name")

		if not existing_item:
			# Also check by item_name
			existing_item = frappe.db.get_value("Item", {"item_name": sku.product_name}, "name")
		...
		# Log sync
		_log_sync_event(
			"SKU Product", sku.name, "Item", item_code, "create" if not existing_item else "update"
		)
		...
		# Log error
		frappe.log_error(message=str(e), title=f"ERPNext Sync Error: SKU Product {sku_product}")
		raise
...
def _create_item_from_sku(sku: Document) -> Document:
	...
	if sku.category:
		item_group = frappe.db.get_value("Item Group", {"item_group_name": sku.category_name}, "name")
...
def sync_item_to_sku(item_code: str, sku_product: str = None, ignore_permissions: bool = False) -> str | None:
	"""
	...
	if not sku_product:
		sku_product = frappe.db.get_value("SKU Product", {"erpnext_item_code": item_code}, "name")

	if not sku_product:
		# Try to find by SKU code matching item_code
		sku_product = frappe.db.get_value("SKU Product", {"sku_code": item_code}, "name")
	...
	try:
		...
		frappe.log_error(message=str(e), title=f"ERPNext Reverse Sync Error: Item {item_code}")
		return None


def bulk_sync_skus_to_items(
	tenant: str = None, limit: int = 100, force_update: bool = False
) -> dict[str, Any]:
	"""
	...
	filters = {"status": ["in", ["Active", "Passive"]], "sync_with_erpnext": 1}
	...
	skus = frappe.get_all(
		"SKU Product", filters=filters, fields=["name"], limit_page_length=limit, order_by="modified desc"
	)

	results = {"success": True, "total": len(skus), "synced": 0, "failed": 0, "errors": []}
	...
	for sku in skus:
		try:
			...
			results["failed"] += 1
			results["errors"].append({"sku_product": sku.name, "error": str(e)})
...
def make_sales_order(source_name: str, target_doc: Document = None) -> Document:
	...
	def set_missing_values(source, target):
		...
		if source.estimated_delivery_date:
			...
			target.delivery_date = frappe.utils.add_days(source.order_date, source.delivery_days or 30)

		# Set company (required for Sales Order)
		if not target.company:
			target.company = frappe.defaults.get_user_default("Company") or frappe.db.get_single_value(
	...
	def update_item(source_item, target_item, source_parent):
		...
		if source_parent.estimated_delivery_date:
			...
			target_item.delivery_date = frappe.utils.add_days(
				source_parent.order_date, source_parent.delivery_days or 30
			)
...
def sync_order_to_sales_order(
	order_name: str, force_update: bool = False, ignore_permissions: bool = False, submit: bool = False
) -> str | None:
	"""
	...
	# Check ERPNext availability
	if not is_erpnext_installed():
		frappe.throw(_("ERPNext is not installed. Cannot sync to Sales Order."), frappe.AppNotInstalledError)
	...
	if not sync_settings.get("enabled") or not sync_settings.get("sync_orders"):
		frappe.msgprint(_("Order sync is not enabled for this tenant"), indicator="yellow")
		return None
	...
	if order.status not in valid_statuses:
		frappe.throw(
			_("Only confirmed orders can be synced to ERPNext. Current status: {0}").format(order.status),
			frappe.ValidationError,
		)
	...
	try:
		...
		_log_sync_event(
			...
			"Sales Order",
			sales_order_name,
			"create" if not existing_sales_order else "update",
		)
		...
		# Log error
		frappe.log_error(message=str(e), title=f"ERPNext Sync Error: Order {order_name}")
		raise
...
def sync_sales_order_to_order(
	sales_order_name: str, order_name: str = None, ignore_permissions: bool = False
) -> str | None:
	"""
	...
	if not order_name:
		...
		order_name = frappe.db.get_value("Order", {"linked_sales_order": sales_order_name}, "name")

	if not order_name:
		# Try to find by PO reference
		sales_order = frappe.get_doc("Sales Order", sales_order_name)
		if sales_order.po_no:
			order_name = frappe.db.get_value("Order", {"name": sales_order.po_no}, "name")
	...
	try:
		...
		frappe.log_error(message=str(e), title=f"ERPNext Reverse Sync Error: Sales Order {sales_order_name}")
		return None
...
def bulk_sync_orders_to_sales_orders(
	...
	orders = frappe.get_all(
		"Order", filters=filters, fields=["name"], limit_page_length=limit, order_by="modified desc"
	)

	results = {"success": True, "total": len(orders), "synced": 0, "failed": 0, "errors": []}
	...
	for order in orders:
		try:
			...
			results["failed"] += 1
			results["errors"].append({"order": order.name, "error": str(e)})
...
def bulk_sync_sellers_to_suppliers(
	...
	for seller in sellers:
		try:
			...
			results["failed"] += 1
			results["errors"].append({"seller": seller.name, "error": str(e)})
...
def bulk_sync_buyers_to_customers(
	...
	for buyer in buyers:
		try:
			...
			results["failed"] += 1
			results["errors"].append({"buyer": buyer.name, "error": str(e)})
...
def _log_sync_event(
	source_doctype: str, source_name: str, target_doctype: str, target_name: str, action: str
) -> None:
	"""
	...
	frappe.get_doc(
		{
			...
			"content": _("ERPNext Sync: {0} {1} to {2}").format(
				action.replace("_", " ").title(), target_doctype, target_name
			),
		}
...
def get_sync_status(doctype: str, docname: str) -> dict[str, Any]:
	...
	if doctype == "Seller Profile":
		...
		return {
			...
			"can_sync": doc.status
			in ["Confirmed", "Processing", "Ready to Ship", "Shipped", "Delivered", "Completed"],
		}
...
def unlink_erpnext_document(doctype: str, docname: str) -> bool:
	...
	if doctype == "Seller Profile":
		...
		frappe.db.set_value("SKU Product", docname, {"erpnext_item_code": None}, update_modified=False)
		return True
	elif doctype == "Order":
		frappe.db.set_value("Order", docname, {"linked_sales_order": None}, update_modified=False)
		return True
...
@frappe.whitelist()
def bulk_sync_all(sync_type: str, tenant: str = None, limit: int = 100) -> dict[str, Any]:
	"""
...
def check_erpnext_availability() -> dict[str, Any]:
	...
	result = {"installed": installed, "doctypes": {}}

	if installed:
		for erpnext_dt, _trade_hub_dt in ERPNEXT_DOCTYPES.items():
...
def enqueue_sync_seller(seller_name: str) -> dict[str, Any]:
	...
	return {"success": True, "message": _("Sync job enqueued for {0}").format(seller_name)}
...
def enqueue_sync_buyer(buyer_name: str) -> dict[str, Any]:
	...
	return {"success": True, "message": _("Sync job enqueued for {0}").format(buyer_name)}
...
def unlink_document(doctype: str, docname: str) -> dict[str, Any]:
	...
	return {"success": success, "message": _("Successfully unlinked") if success else _("Failed to unlink")}
...
def enqueue_sync_sku(sku_product: str) -> dict[str, Any]:
	...
	return {"success": True, "message": _("Sync job enqueued for {0}").format(sku_product)}
...
@frappe.whitelist()
def bulk_sync_skus(tenant: str = None, limit: int = 100) -> dict[str, Any]:
	"""
...
def create_item_from_sku(sku_product: str) -> dict[str, Any]:
	...
	"""
	if not is_erpnext_installed():
		return {"success": False, "error": _("ERPNext is not installed")}

	if not frappe.db.exists("SKU Product", sku_product):
		return {"success": False, "error": _("SKU Product {0} not found").format(sku_product)}

	try:
		item = make_item_from_sku(sku_product)
		return {"success": True, "item": item.as_dict()}
	except Exception as e:
		return {"success": False, "error": str(e)}
...
def sync_order(order_name: str, force: bool = False, submit: bool = False) -> dict[str, Any]:
	...
	try:
		sales_order_name = sync_order_to_sales_order(
			order_name, force_update=cint(force), submit=cint(submit)
		)
		return {
			"success": True,
...
def enqueue_sync_order(order_name: str, submit: bool = False) -> dict[str, Any]:
	...
	return {"success": True, "message": _("Sync job enqueued for {0}").format(order_name)}
...
def get_order_sync_status(order_name: str) -> dict[str, Any]:
	...
	return {
		...
		"can_sync": order.status
		in ["Confirmed", "Processing", "Ready to Ship", "Shipped", "Delivered", "Completed"],
	}


@frappe.whitelist()
def bulk_sync_orders(tenant: str = None, limit: int = 100) -> dict[str, Any]:
	"""
...
def create_sales_order_from_order(order_name: str) -> dict[str, Any]:
	...
	if not frappe.db.exists("Order", order_name):
		return {"success": False, "error": _("Order {0} not found").format(order_name)}

	try:
		sales_order = make_sales_order(order_name)
		return {"success": True, "sales_order": sales_order.as_dict()}
	except Exception as e:
		return {"success": False, "error": str(e)}
...
def trigger_order_sync_webhook(
	order_name: str, sales_order_name: str, event: str = "sync", data: dict[str, Any] = None
) -> bool:
	"""
	...
	for endpoint in webhook_endpoints:
		try:
			...
			frappe.log_error(
				message=f"Webhook failed: {endpoint}\n{str(e)}", title=f"Order Webhook Error: {order_name}"
			)
			success = False
...
def _get_order_webhook_endpoints(tenant: str) -> list[str]:
	...
	webhooks = frappe.get_all(
		"Webhook", filters={"webhook_doctype": "Order", "enabled": 1}, fields=["name", "request_url"]
	)
...
def handle_sales_order_webhook() -> dict[str, Any]:
	...
	try:
		...
		data = payload.get("data", {})  # noqa: F841  # reserved for future webhook payload handling

		# Validate payload
		if doctype != "Sales Order":
			return {"success": False, "error": f"Unexpected doctype: {doctype}"}
		...
		if event in ["on_update", "on_submit", "on_change"]:
			...
			order_name = sync_sales_order_to_order(docname, ignore_permissions=True)
			...
			if order_name:
				...
				return {"success": True, "message": f"No linked Order found for Sales Order {docname}"}
			...
			return {"success": True, "message": f"Unhandled event type: {event}"}

	except json.JSONDecodeError:
		return {"success": False, "error": "Invalid JSON payload"}
	except Exception as e:
		frappe.log_error(message=str(e), title="Sales Order Webhook Error")
		return {"success": False, "error": str(e)}
...
def _handle_sales_order_cancellation(sales_order_name: str) -> str | None:
	"""
	...
	if order.status not in ["Completed", "Cancelled", "Refunded"]:
		...
		order.add_comment(
			"Info", _("Linked Sales Order {0} was cancelled in ERPNext").format(sales_order_name)
		)
...
@frappe.whitelist()
def register_order_webhook(webhook_url: str, events: list[str] = None, tenant: str = None) -> dict[str, Any]:
	"""
	...
	try:
		...
		for field in ["name", "status", "buyer", "tenant", "total_amount", "currency"]:
			webhook.append("webhook_data", {"fieldname": field, "key": field})

		webhook.insert(ignore_permissions=True)

		return {"success": True, "webhook": webhook.name, "message": _("Webhook registered successfully")}

	except Exception as e:
		frappe.log_error(message=str(e), title="Webhook Registration Error")
		return {"success": False, "error": str(e)}
...
def get_order_webhook_status(order_name: str = None) -> dict[str, Any]:
	...
	if result["webhooks_enabled"]:
		...
		if frappe.db.exists("DocType", "Webhook Request Log"):
			logs = frappe.get_all(
				"Webhook Request Log",
				filters={"reference_doctype": "Order"},
...
def sync_order_with_webhook(
	order_name: str, force: bool = False, submit: bool = False, trigger_webhook: bool = True
) -> dict[str, Any]:
	"""
	...
	try:
		...
		if not sales_order_name:
			return {"success": False, "error": "Sync failed or not enabled"}
		...
		if trigger_webhook:
			webhook_success = trigger_order_sync_webhook(order_name, sales_order_name, event="sync")
