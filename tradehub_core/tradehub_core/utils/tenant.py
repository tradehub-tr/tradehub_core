...
def get_user_tenant(user: str) -> str | None:
	"""
	...
	if not tenant:
		user_permissions = frappe.get_all(
			"User Permission",
			filters={"user": user, "allow": "Tenant", "is_default": 1},
			pluck="for_value",
			limit=1,
		)
		if user_permissions:
	...
	if not tenant:
		user_permissions = frappe.get_all(
			"User Permission", filters={"user": user, "allow": "Tenant"}, pluck="for_value", limit=1
		)
		if user_permissions:
			tenant = user_permissions[0]
...
def set_tenant_context(tenant: str) -> None:
	...
	if tenant:
		...
		if not frappe.db.exists("Tenant", tenant):
			frappe.throw(_("Tenant {0} does not exist").format(tenant), frappe.ValidationError)

		# Check if tenant is active
		tenant_status = frappe.db.get_value("Tenant", tenant, "status")
		if tenant_status not in ["Active", "Trial"]:
			frappe.throw(
				_("Tenant {0} is not active (status: {1})").format(tenant, tenant_status),
...
def check_tenant_permission(tenant: str, user: str = None, ptype: str = "read") -> bool:
	"""
...
def switch_tenant(tenant_name: str) -> dict:
	...
	"""
	if not is_platform_admin():
		frappe.throw(_("Only platform administrators can switch tenants"), frappe.PermissionError)
