...
| En az bir teste bağlı (A ∪ C ∪ F) | **86** (%42.6) |
| **KAPSANMIYOR** | **116** (%57.4) |
| A kanıtı (test metninde kimlik geçiyor) | 38 |
...
| Taranan Python test dosyası | 185 |
| Taranan Python test fonksiyonu | 3912 |
| Taranan FE test dosyası | 207 (node 110 · vitest 62 · e2e 31 · panel-e2e 4) |
| Taranan FE testi (koşan) | 1766 (node 1256 · vitest 304 · e2e 196 · panel-e2e 10) |
| Atlanan FE testi (skip/fixme/todo — kapsam dışı) | 7 |
...
gereksinim sayısı **116**. Liste §3'te tam olarak yazılı.
...
| FR | Gereksinim (kısaltılmış) | Faz | SRS'teki bugünkü durum | Bağlayıcı test (A/C/F) | Fixture izi (B — sayılmaz) |
|---|---|---|---|---|---|
...
| **FR-009** | Sistem, MIME/uzantı kararını içerik imzasına (magic byte) göre vermeli ve uzantı ile içerik uyuşmazlığında reddetmelidir. | F3 | KISMEN — L0'da uyuşmazlık yalnız uyarı; magic-byte yalnız… | `tradehub_core/tests/test_contracts.py`<br>`tradehub_core/tests/test_contracts.py::PolicyContractTest::test_uzanti_icerik_uyusmazligi` | `tradehub_core/tests/test_api_contracts.py::YuklemeTesti::test_politika_ihlali_422_ve_slot_onekli_kod`<br>`tradehub_core/tests/test_image_probe.py`<br>`tradehub_core/tests/test_image_probe.py::KotucuIcerikTest::test_kesik_dosya_baslikta_gecerli_ama_reddedilir`<br>`tradehub_core/tests/test_isolation.py::GercekIkiliyleEntegrasyon::test_ffprobe_bozuk_dosyada_temiz_hata_verir`<br>…+3 iz |
...
| **FR-011** | Sistem, decompression bomb koruması olarak, görselin tamamı açılmadan yalnız başlıktan piksel sayısını okumalı ve `accept.max_megapixels_hard`… | F3 | YOK — `grep MAX_IMAGE_PIXELS tradehub_core/` → 0 sonuç;… | `tradehub_core/tests/test_contracts.py::ImageContractTest::test_probe_megapiksel_bombasini_reddeder`<br>`tradehub_core/tests/test_e2e_scenarios.py`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo03PikselTavani::test_megapiksel_karari_bayttan_bagimsiz`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo03PikselTavani::test_piksel_acilmadan_reddedilebiliyor`<br>…+5 test | `tradehub_core/tests/test_e2e_scenarios.py`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo10KotuculDosya::test_bomba_piksel_acilmadan_reddedilir`<br>`tradehub_core/tests/test_image_lqip.py::SozlesmeTest::test_72mp_kaynak`<br>`tradehub_core/tests/test_image_normalize.py::BellekButcesiTest::test_72mp_jpeg_decode_asamasinda_kuculur_peak_bellek_500mb_altinda`<br>…+12 iz |
...
| **FR-015** | Sistem, `require.min_short_edge` ve `require.min_area` kontrollerini `>=` (eşitlik geçerli) ile uygulamalıdır. | F3 | YOK — 0 slotta piksel kuralı var | `tradehub_core/tests/test_contracts.py::PolicyContractTest::test_geometri_esitlik_gecerlidir`<br>`tradehub_core/tests/test_e2e_scenarios.py`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo01YetersizCozunurluk::test_1000_piksel_gecer_sinir_dahil`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo01YetersizCozunurluk::test_999_piksel_reddedilir`<br>…+4 test | `tradehub_core/tests/test_e2e_scenarios.py`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo01YetersizCozunurluk::test_1000_piksel_gecer_sinir_dahil`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo01YetersizCozunurluk::test_999_piksel_reddedilir`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo01YetersizCozunurluk::test_satici_ne_yapacagini_ogrenir`<br>…+12 iz |
| **FR-016** | Sistem, en-boy oranı kontrolünü bağıl tolerans ile uygulamalıdır: kabul ⟺ `min over r ∈ allowed_ratios of \ | F3 | YOK — oranı yazan tek yer bir önizleme CSS'i… | `tradehub_core/tests/test_contracts.py::PolicyContractTest::test_geometri_bagil_oran_toleransi`<br>`tradehub_core/tests/test_e2e_scenarios.py`<br>`node: admin-panel/frontend/src/lib/media/policy/__tests__/policyEngine.test.js :: oran toleransı BAĞIL: bant içi geçer, bant dışı ret`<br>`node: admin-panel/frontend/src/lib/media/upload/__tests__/preflight.test.js :: izinsiz oran reddediliyor, tolerans içi geçiyor`<br>…+1 test | `tradehub_core/tests/test_image_classify.py`<br>`tradehub_core/tests/test_image_classify.py::FixtureAyrimTest::test_ayrilanlar_manifest_fixture_aileleridir`<br>`tradehub_core/tests/test_image_classify.py::KapiTest::test_skip_guard_ile_kapi_atlanir`<br>`tradehub_core/tests/test_image_classify.py::SozlesmeTest::test_to_dict_serilestirilebilir`<br>…+22 iz |
...
| **FR-018** | Sistem, `require.max_count` ile galeri adet sınırını uygulamalıdır. | F3 | YOK — hiçbir galeri slotunda adet kuralı yok | `tradehub_core/tests/test_contracts.py::PolicyContractTest::test_geometri_kisa_kenar_ve_adet`<br>`node: admin-panel/frontend/src/lib/media/upload/__tests__/preflight.test.js :: max_count aşımı reddediliyor` | — |
| **FR-019** | Sistem, logo slotlarında alfa kanalını tercih etmeli, alfası olmayan dosyayı kabul edip uyarmalıdır — reddetmemelidir. Alfasızlık ölçülmeli ve kayıt… | F3 | YOK | **KAPSANMIYOR** | `tradehub_core/tests/test_image_classify.py`<br>`tradehub_core/tests/test_image_lqip.py::AlfaTest::test_alfali_fixturelar_alfa_bayragi_tasir`<br>`tradehub_core/tests/test_image_lqip.py::AlfaTest::test_baskin_renk_saydam_bolgeyi_saymaz`<br>`tradehub_core/tests/test_image_lqip.py::AlfaTest::test_saydam_bolge_geri_acildiginda_saydam`<br>…+7 iz |
| **FR-020** | Sistem, logo master'ını kabul bandındaki (`1:2 … 2:1`) her orandan 1:1'e saydam letterbox ile normalize etmeli; asla kırpmamalıdır. | F3 | YOK | `tradehub_core/tests/test_policy_engine.py::NormalizeHedefTesti::test_logo_masteri_kare_paddir` | `tradehub_core/tests/test_image_lqip.py::AlfaTest::test_alfali_fixturelar_alfa_bayragi_tasir`<br>`tradehub_core/tests/test_image_lqip.py::AlfaTest::test_baskin_renk_saydam_bolgeyi_saymaz`<br>`tradehub_core/tests/test_image_lqip.py::AlfaTest::test_saydam_bolge_geri_acildiginda_saydam`<br>`tradehub_core/tests/test_image_lqip.py::HizTest::test_kucuk_kaynakta_uctan_uca_da_butce_icinde`<br>…+3 iz |
| **FR-021** | Sistem, `user.avatar` slotunda `1:1` oranı zorunlu tutmalı (tolerans ±%2) ve dışını reddetmelidir. | F3 | YOK — bugün sessizce merkezden kırpılıyor | **KAPSANMIYOR** | `tradehub_core/tests/test_image_normalize.py::PikselTavaniTest::test_kucuk_gorsel_buyutulmez` |
...
| **FR-028** | Sistem asla upscale yapmamalıdır; master üretimi yalnız küçültme yönünde çalışmalıdır. | F1 ✅ | VAR — korunacak | `tradehub_core/tests/test_api_contracts.py::ManifestUreticiTesti::test_pick_hicbiri_yetmezse_en_buyugu`<br>`tradehub_core/tests/test_contracts.py::ImageContractTest::test_master_yalniz_kucultur`<br>`tradehub_core/tests/test_e2e_scenarios.py`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo03PikselTavani::test_tavan_uygulanınca_master_kuculur`<br>…+6 test | — |
...
| **FR-049** | Sistem, birden çok kural tetiklendiğinde en yüksek aksiyonu uygulamalı, uyarıları birikimli listelemelidir. | F3+ | YOK | `tradehub_core/tests/test_contracts.py::PolicyContractTest::test_karar_birlesimi_en_yuksek_aksiyonu_alir`<br>`node: admin-panel/frontend/src/lib/media/policy/__tests__/policyEngine.test.js :: birden çok ihlalde EN YÜKSEK aksiyon kazanır, uyarılar düşmez` | — |
...
| **FR-060** | Sistem, her reddi `(kod, retryable)` çifti olarak döndürmeli; istemci karar verirken hata metnine değil koda bakmalıdır. | F1 ✅ | VAR (çekirdek) — korunacak | `tradehub_core/tests/test_contracts.py::PolicyContractTest::test_hata_yaniti_kod_ve_retryable_tasir`<br>`node: admin-panel/frontend/src/lib/media/upload/__tests__/session.test.js :: politika reddi YENİDEN DENENMİYOR — aynı dosya aynı cevabı verir`<br>`node: admin-panel/frontend/src/utils/__tests__/uploadPolicy.test.js :: politika reddi yeniden denenmiyor, geçici hata deneniyor` | — |
| **FR-061** | Sistem, kodlu ret sözleşmesini tüm yükleme uçlarında kullanmalıdır. | F3 | KISMEN | **KAPSANMIYOR** | — |
| **FR-062** | Sistem, her kullanıcı mesajında iki soruyu yanıtlamalıdır: (1) NEDEN reddedildi, (2) NASIL düzeltilir. | F2 ✅ | VAR (şema düzeyinde) | `tradehub_core/tests/test_e2e_scenarios.py`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo01YetersizCozunurluk::test_satici_ne_yapacagini_ogrenir`<br>`panel-e2e: admin-panel/frontend/tests/e2e/media-upload-security.spec.ts :: S1 — 1000×1000 altı görsel reddedilir ve düzeltici yönlendirme gösterili…` | — |
...
| **FR-105** | Sistem, uygulanmamış bir depolama hedefi seçildiğinde politikayı reddetmelidir. | F3+ | YOK | `tradehub_core/tests/test_retention.py::TestPolitikaDogrulama::test_bilinmeyen_hedef_reddedilir` | — |
| **FR-106** | Sistem, yedek alma ile temizlik sırasını korumalı: ÖNCE yedek, SONRA eskileri temizle. | F1 ✅ | VAR — korunacak | **KAPSANMIYOR** | — |
| **FR-107** | Sistem, disk ile veritabanı arasındaki yetim/kırık kayıtları düzenli olarak uzlaştırmalıdır. | F3+ | KISMEN | `tradehub_core/tests/test_usage.py::DiskOksuzuTesti::test_kayitsiz_ve_eski_dosya_oksuz`<br>`tradehub_core/tests/test_usage.py::KayitOksuzuTesti::test_bagi_olmayan_eski_varlik_oksuz` | — |
...
| **FR-121** | Sistem, görseli `srcset`/`sizes`/`<picture>` ile cihaza uygun boyda servis etmelidir. | F3+ | YOK | `tradehub_core/tests/test_e2e_scenarios.py`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo04KirpmaVeCihazOnizleme::test_tum_cihaz_sinifi_kombinasyonlari_cozuluyor`<br>`tradehub_core/tests/test_simulator_srcset.py::SizesUretimi::test_her_bolge_sizes_uretebiliyor`<br>`tradehub_core/tests/test_simulator_srcset.py::SizesUretimi::test_srcset_dizgesi_artan_genislikte`<br>…+8 test | — |
...
| **FR-123** | Sistem, bugünkü aşırı-servis oranını azaltmalıdır. | F3+ | YOK | `tradehub_core/tests/test_contracts.py::DeliveryContractTest::test_overshoot_olculur`<br>`e2e: tradehubfront/tests/e2e/media-delivery-manifest.spec.ts :: S3 vitrin yarısı — piksel tavanı uygulanmış türev bildirildiğinde ürün s…` | — |
| **FR-124** | Sistem, CLS'i önlemek için her görsel kutusunda oran veya sabit ölçü rezerve etmelidir. | F3+ | KISMEN | `tradehub_core/tests/test_api_contracts.py::TeslimTesti::test_icsel_olcu_tasiniyor`<br>`e2e: tradehubfront/tests/e2e/media-delivery-manifest.spec.ts :: S2 vitrin yarısı — 2400×2400 türev içsel ölçüsüyle ve tavana kadar srcse…`<br>`node: admin-panel/frontend/src/components/media/__tests__/mediaCls.test.js :: kaynak yokken kutu yine ayrılır — boş satır zıplamasın`<br>`node: admin-panel/frontend/src/components/media/__tests__/mediaCls.test.js :: medya ekranlarındaki her görsel kutusu ÖNCEDEN ayrılmış`<br>…+6 test | — |
...
| **NFR-041** | Yarım dosya asla yazılmamalıdır. | F1 ✅ | VAR — korunacak | `tradehub_core/tests/test_e2e_scenarios.py`<br>`tradehub_core/tests/test_video_transcode.py::GercekTranscode::test_yarim_dosya_diskte_kalmaz` | — |
...
| **NFR-050** | Aynı içerik tek fiziksel dosya olmalıdır. | F1 ✅ | VAR — korunacak | `tradehub_core/tests/test_e2e_scenarios.py`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo09AyniDosyaIkiKez::test_ikinci_yukleme_yeni_varlik_uretmez`<br>`tradehub_core/tests/test_media_pipeline_integration.py::TestHashDedup::test_ayni_icerik_iki_kez_yuklenince_ayni_file_url_uretir`<br>`panel-e2e: admin-panel/frontend/tests/e2e/media-dedup.spec.ts :: S9 — aynı dosya iki kez yüklendiğinde tek asset kalır (dedup)` | — |
| **NFR-051** | Dizin başına dosya sayısı ölçeklenebilir kalmalıdır. | F1 ✅ | VAR — korunacak | `tradehub_core/tests/test_e2e_scenarios.py`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo09AyniDosyaIkiKez::test_adres_icerikten_turer`<br>`tradehub_core/tests/test_dedup.py::AdlandirmaSozlesmesiTesti::test_ad_ve_shard_bicimi` | — |
...
Aşağıdaki **116** gereksinimin bu depoda hiçbir testi yok.
Gizlenmedi, sayıldı:
