...
| Normatif invariant | **12** (INV-01…INV-12) |
| Toplam izlenebilir öğe | **214** |
| En az bir teste bağlı (A ∪ C ∪ F ∪ I) | **99** (%46.3) |
| **KAPSANMIYOR** | **115** (%53.7) |
| A kanıtı (test metninde kimlik geçiyor) | 52 |
| C kanıtı (yalnız elle eşleme, A yok) | 35 |
| F kanıtı — FE testi olan gereksinim | 22 |
| …bunlardan YALNIZ FE ile kapsanan (A/C yok) | 10 |
| I kanıtı — doğrulanmış invariant testi | 12/12 |
| B izi olan gereksinim (kapsam SAYILMAZ) | 24 |
| …bunlardan yalnız B izi olan, yani hâlâ kapsanmayan | 7 |
| Taranan Python test dosyası | 210 |
| Taranan Python test fonksiyonu | 4105 |
| Taranan FE test dosyası | 214 (node 116 · vitest 62 · e2e 31 · panel-e2e 5) |
| Taranan FE testi (koşan) | 1812 (node 1297 · vitest 304 · e2e 196 · panel-e2e 15) |
| Atlanan FE testi (skip/fixme/todo — kapsam dışı) | 6 |
| FE test adlarındaki `[FR/NFR-xxx]` etiketi | 51 |

**Kabul kriteri karşılığı (kaynak doküman T-140):**

- Her FR/NFR en az bir teste bağlı: **87/202**; açık **115** → **SAĞLANMIYOR**.
- INV-01…INV-12 property/golden teste bağlı: **12/12**; açık **0** → **SAĞLANDI**.
- Açık öğelerin tam listesi §3'tedir.
...
| **F** | FE test adının BAŞINDA `[FR-xxx]` etiketi (node --test / vitest / playwright); koşucu adı satırda görünür, `skip` edilen test sayılmaz | bağlayıcı |
| **I** | `tests/golden/invariants.json` içindeki birincil/destek test sembolü; Python AST indeksiyle varlığı doğrulandı | bağlayıcı |
...
| FR | Gereksinim (kısaltılmış) | Faz | SRS'teki bugünkü durum | Bağlayıcı test (A/C/F/I) | Fixture izi (B — sayılmaz) |
|---|---|---|---|---|---|
...
| **FR-002** | Sistem, slot politikalarını tek bir kayıt defterinden okumalı ve tüm politika dosyaları tek bir şemaya uymalıdır. | F3 | YOK — 4 ayrı kayıt yeri, 3 farklı biçim | `tradehub_core/tests/test_contracts.py::PolicyContractTest::test_dokuz_slot_yuklendi`<br>`tradehub_core/tests/test_policy_engine.py::PolitikaKaydiTesti::test_dokuz_slot_politikasi_yuklendi` | — |
| **FR-003** | Sistem, politika şeması doğrulamasını CI'da çalıştırmalı ve hata varsa derlemeyi başarısız saymalıdır. | F3 | YOK — betik var, CI bağlantısı yok | `tradehub_core/tests/test_contracts.py::PolicyContractTest::test_dogrulama_d1_d5` | — |
...
| **FR-005** | Sistem, bir politikayı yalnız `open_questions` boş VE hiçbir `encoder_quality` `null` değil iken `status: "active"` kabul etmelidir; `draft` politika… | F3 | YOK — kapı tanımlı, uygulayan kod yok | `tradehub_core/tests/test_faz2_closure.py::Faz2PolitikaKapisiTesti::test_encoder_quality_null_degil`<br>`tradehub_core/tests/test_faz2_closure.py::Faz2PolitikaKapisiTesti::test_standartlar_sabit_ve_acik_sorusuz` | — |
...
| **FR-007** | Sistem, bir slot politikasının global tavanı gevşetmesine izin vermemelidir: efektif tavan `min(slot.accept.max_bytes, upload_policy.MAX_BYTES[kind],… | F3 | KISMEN — `min(bizim, platform_limit())` var, plan ve slot… | `tradehub_core/tests/test_contracts.py::PolicyContractTest::test_efektif_tavan_kesisimdir` | — |
| **FR-008** | Sistem, `accept.extensions` listesi ile `accept.mime` listesi ayrıştığında politikayı sessizce delinmiş saymamalı, hata bildirmelidir. | F3 | YOK | **KAPSANMIYOR** | `tradehub_core/tests/test_api_contracts.py::YuklemeTesti::test_politika_ihlali_422_ve_slot_onekli_kod`<br>`tradehub_core/tests/test_image_probe.py`<br>`tradehub_core/tests/test_media_security_gate.py`<br>`tradehub_core/tests/test_policy_engine.py::KuralDavranisiTesti::test_uzanti_icerik_uyusmazligi_reddedilir` |
| **FR-009** | Sistem, MIME/uzantı kararını içerik imzasına (magic byte) göre vermeli ve uzantı ile içerik uyuşmazlığında reddetmelidir. | F3 | KISMEN — L0'da uyuşmazlık yalnız uyarı; magic-byte yalnız… | `tradehub_core/tests/test_contracts.py::PolicyContractTest::test_uzanti_icerik_uyusmazligi` | `tradehub_core/tests/test_api_contracts.py::YuklemeTesti::test_politika_ihlali_422_ve_slot_onekli_kod`<br>`tradehub_core/tests/test_image_probe.py`<br>`tradehub_core/tests/test_image_probe.py::KotucuIcerikTest::test_kesik_dosya_baslikta_gecerli_ama_reddedilir`<br>`tradehub_core/tests/test_isolation.py::GercekIkiliyleEntegrasyon::test_ffprobe_bozuk_dosyada_temiz_hata_verir`<br>…+3 iz |
| **FR-010** | Sistem, `.docx` beyan eden bir dosyanın gerçekten DOCX olduğunu ZIP içeriğinden doğrulamalıdır. | F3 | KISMEN — yalnız KYB ucunda | **KAPSANMIYOR** | `tradehub_core/tests/test_image_probe.py`<br>`tradehub_core/tests/test_media_security_gate.py` |
| **FR-011** | Sistem, decompression bomb koruması olarak, görselin tamamı açılmadan yalnız başlıktan piksel sayısını okumalı ve `accept.max_megapixels_hard`… | F3 | YOK — `grep MAX_IMAGE_PIXELS tradehub_core/` → 0 sonuç;… | `node: admin-panel/frontend/src/lib/media/upload/__tests__/preflight.test.js :: PNG boyutu başlıktan okunuyor — dosya ÇÖZÜLMEDEN`<br>`node: admin-panel/frontend/src/lib/media/upload/__tests__/preflight.test.js :: megapiksel tavanı `>` ile reddediyor — eşitlik geçerli`<br>`panel-e2e: admin-panel/frontend/tests/e2e/media-upload-security.spec.ts :: S10 — bomb / polyglot / SVG-XSS reddedilir ve worker ayakta kalır`<br>`tradehub_core/tests/test_contracts.py::ImageContractTest::test_probe_megapiksel_bombasini_reddeder`<br>…+5 test | `tradehub_core/tests/golden/test_performance_budgets.py::PerformanceBudgetAAA::test_representative_fixture_in_each_process_class_stays_in_budget`<br>`tradehub_core/tests/test_e2e_scenarios.py`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo10KotuculDosya::test_bomba_piksel_acilmadan_reddedilir`<br>`tradehub_core/tests/test_image_lqip.py::SozlesmeTest::test_72mp_kaynak`<br>…+13 iz |
| **FR-012** | Sistem, `allow_animated: false` olan slotlarda animasyonlu görseli reddetmelidir; `auto_fix` seçilen slotta ilk kareyi alıp statik türev üretmelidir. | F3 | HATALI — motor atlıyor (`reason="animated"`), dosya kabul… | `node: admin-panel/frontend/src/lib/media/upload/__tests__/preflight.test.js :: animasyon yasağı yalnız animasyon ÖLÇÜLDÜYSE işliyor` | `tradehub_core/tests/test_image_lqip.py::SozlesmeTest::test_animasyonda_ilk_kare` |
| **FR-013** | Sistem, medya alanına `data:` URI yazılmasını reddetmelidir; medya bir dosya olarak yüklenmelidir. | F3 | YOK — kanal açık; `seed_demo_data.py:231-245` iki güvenlik… | **KAPSANMIYOR** | `tradehub_core/tests/test_image_probe.py`<br>`tradehub_core/tests/test_media_security_gate.py` |
| **FR-014** | Sistem, `.svg` ve `.svgz` yüklemelerini SVG-1…SVG-10 ön koşullarının tamamı karşılanana kadar reddetmeye devam etmelidir. | F3+ | VAR (iki kapı reddediyor) — korunacak | `panel-e2e: admin-panel/frontend/tests/e2e/media-upload-security.spec.ts :: S10 — bomb / polyglot / SVG-XSS reddedilir ve worker ayakta kalır` | `tradehub_core/tests/test_image_probe.py`<br>`tradehub_core/tests/test_media_security_gate.py`<br>`tradehub_core/tests/test_policy_engine.py::KuralDavranisiTesti::test_kosullu_svg_kapali_oldugu_icin_reddedilir`<br>`tradehub_core/tests/test_svg_sanitize.py` |
| **FR-015** | Sistem, `require.min_short_edge` ve `require.min_area` kontrollerini `>=` (eşitlik geçerli) ile uygulamalıdır. | F3 | YOK — 0 slotta piksel kuralı var | `node: admin-panel/frontend/src/lib/media/policy/__tests__/policyEngine.test.js :: eşitlik GEÇERLİDİR: kısa kenar tam eşikte ihlal üretmez`<br>`node: admin-panel/frontend/src/lib/media/upload/__tests__/preflight.test.js :: kısa kenar `>=` ile geçerli`<br>`panel-e2e: admin-panel/frontend/tests/e2e/media-upload-security.spec.ts :: S1 — 1000×1000 altı görsel reddedilir ve düzeltici yönlendirme gösterili…`<br>`panel-e2e: admin-panel/frontend/tests/e2e/media-upload-security.spec.ts :: S1b — sunucu kapısı: upload_media 1000×1000 altını reddediyor (W7)`<br>…+4 test | `tradehub_core/tests/golden/test_perceptual.py`<br>`tradehub_core/tests/test_e2e_scenarios.py`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo01YetersizCozunurluk::test_1000_piksel_gecer_sinir_dahil`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo01YetersizCozunurluk::test_999_piksel_reddedilir`<br>…+13 iz |
| **FR-016** | Sistem, en-boy oranı kontrolünü bağıl tolerans ile uygulamalıdır: kabul ⟺ `min over r ∈ allowed_ratios of \ | F3 | YOK — oranı yazan tek yer bir önizleme CSS'i… | `node: admin-panel/frontend/src/lib/media/policy/__tests__/policyEngine.test.js :: oran toleransı BAĞIL: bant içi geçer, bant dışı ret`<br>`node: admin-panel/frontend/src/lib/media/upload/__tests__/preflight.test.js :: izinsiz oran reddediliyor, tolerans içi geçiyor`<br>`node: admin-panel/frontend/src/lib/media/upload/__tests__/preflight.test.js :: sapma EN YAKIN hedefe göre ve BAĞIL — sunucudaki formülün aynısı`<br>`tradehub_core/tests/test_contracts.py::PolicyContractTest::test_geometri_bagil_oran_toleransi`<br>…+1 test | `tradehub_core/tests/test_image_classify.py`<br>`tradehub_core/tests/test_image_classify.py::FixtureAyrimTest::test_ayrilanlar_manifest_fixture_aileleridir`<br>`tradehub_core/tests/test_image_classify.py::KapiTest::test_skip_guard_ile_kapi_atlanir`<br>`tradehub_core/tests/test_image_classify.py::SozlesmeTest::test_to_dict_serilestirilebilir`<br>…+23 iz |
...
| **FR-018** | Sistem, `require.max_count` ile galeri adet sınırını uygulamalıdır. | F3 | YOK — hiçbir galeri slotunda adet kuralı yok | `node: admin-panel/frontend/src/lib/media/upload/__tests__/preflight.test.js :: max_count aşımı reddediliyor`<br>`tradehub_core/tests/test_contracts.py::PolicyContractTest::test_geometri_kisa_kenar_ve_adet` | — |
| **FR-019** | Sistem, logo slotlarında alfa kanalını tercih etmeli, alfası olmayan dosyayı kabul edip uyarmalıdır — reddetmemelidir. Alfasızlık ölçülmeli ve kayıt… | F3 | YOK | **KAPSANMIYOR** | `tradehub_core/tests/golden/test_perceptual.py`<br>`tradehub_core/tests/test_image_classify.py`<br>`tradehub_core/tests/test_image_lqip.py::AlfaTest::test_alfali_fixturelar_alfa_bayragi_tasir`<br>`tradehub_core/tests/test_image_lqip.py::AlfaTest::test_baskin_renk_saydam_bolgeyi_saymaz`<br>…+8 iz |
| **FR-020** | Sistem, logo master'ını kabul bandındaki (`1:2 … 2:1`) her orandan 1:1'e saydam letterbox ile normalize etmeli; asla kırpmamalıdır. | F3 | YOK | `tradehub_core/tests/test_policy_engine.py::NormalizeHedefTesti::test_logo_masteri_kare_paddir` | `tradehub_core/tests/golden/test_perceptual.py`<br>`tradehub_core/tests/test_image_lqip.py::AlfaTest::test_alfali_fixturelar_alfa_bayragi_tasir`<br>`tradehub_core/tests/test_image_lqip.py::AlfaTest::test_baskin_renk_saydam_bolgeyi_saymaz`<br>`tradehub_core/tests/test_image_lqip.py::AlfaTest::test_saydam_bolge_geri_acildiginda_saydam`<br>…+4 iz |
| **FR-021** | Sistem, `user.avatar` slotunda `1:1` oranı zorunlu tutmalı (tolerans ±%2) ve dışını reddetmelidir. | F3 | YOK — bugün sessizce merkezden kırpılıyor | **KAPSANMIYOR** | `tradehub_core/tests/golden/test_perceptual.py`<br>`tradehub_core/tests/test_image_normalize.py::PikselTavaniTest::test_kucuk_gorsel_buyutulmez` |
...
| **FR-023** | Sistem, `company.cover_image` slotunda güvenli alanı merkez %41,7'lik dikey şerit olarak uygulamalı ve dışına taşan içerik için uyarı üretmelidir. | F3+ | YOK | `node: admin-panel/frontend/src/lib/media/crop/__tests__/cropPolicy.test.js :: uyarı türlerinin her biri en az bir fixture'da üretiliyor`<br>`node: admin-panel/frontend/src/lib/media/crop/__tests__/cropSafeArea.test.js :: güvenli alan eşikleri canlı politikayla birebir`<br>`node: admin-panel/frontend/src/lib/media/crop/__tests__/cropSafeArea.test.js :: kadrajın merkezindeki odak bandın içindedir`<br>`node: admin-panel/frontend/src/lib/media/crop/__tests__/cropSafeArea.test.js :: kuralı olmayan slotta bant YOK — eşik uydurulmuyor` | `tradehub_core/tests/test_image_lqip.py::OranTest::test_yatay_ve_dikey_ayirt_edilir`<br>`tradehub_core/tests/test_policy_engine.py::NormalizeHedefTesti::test_megapiksel_tavani_uzun_kenardan_once_baglar` |
| **FR-024** | Sistem, `category.banner` slotunda güvenli alanı merkez %42 × %42 olarak uygulamalıdır. | F3+ | YOK | `node: admin-panel/frontend/src/lib/media/crop/__tests__/cropSafeArea.test.js :: güvenli alan eşikleri canlı politikayla birebir`<br>`node: admin-panel/frontend/src/lib/media/crop/__tests__/cropSafeArea.test.js :: kuralı olmayan slotta bant YOK — eşik uydurulmuyor` | `tradehub_core/tests/test_image_lqip.py::DogrulukTest::test_geri_acilan_hash_ortalama_renge_yakin`<br>`tradehub_core/tests/test_image_lqip.py::OranTest::test_oran_gercek_orana_yakin`<br>`tradehub_core/tests/test_policy_engine.py::KuralDavranisiTesti::test_ignore_aksiyonu_kullaniciya_gosterilmez` |
...
| **FR-027** | Sistem, belge slotunda geometri ihlalini `warn` ile karşılamalı, reddetmemelidir. | F3 | YOK (uyarı da yok) | `tradehub_core/tests/test_contracts.py::PolicyContractTest::test_belge_slotunda_geometri_uyaridir` | — |
| **FR-028** | Sistem asla upscale yapmamalıdır; master üretimi yalnız küçültme yönünde çalışmalıdır. | F1 ✅ | VAR — korunacak | `node: admin-panel/frontend/src/lib/media/simulator/__tests__/srcsetParity.test.js :: profil merdiveni politikadan aynı çıkıyor (upscale kelepçesi dahil)`<br>`tradehub_core/tests/test_api_contracts.py::ManifestUreticiTesti::test_pick_hicbiri_yetmezse_en_buyugu`<br>`tradehub_core/tests/test_contracts.py::ImageContractTest::test_master_yalniz_kucultur`<br>`tradehub_core/tests/test_e2e_scenarios.py`<br>…+6 test | — |
| **FR-029** | Sistem, DPI değişimini asla piksel kaybına çevirmemelidir. | F2 ✅ | VAR (fiilen) | `tradehub_core/tests/test_contracts.py::ImageContractTest::test_master_dpi_yazar_pikseli_degistirmez`<br>`tradehub_core/tests/test_e2e_scenarios.py`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo02DpiPikselDusurmez::test_3000_300dpi_2400_72dpiye_iner` | `tradehub_core/tests/test_e2e_scenarios.py`<br>`tradehub_core/tests/test_image_normalize.py`<br>`tradehub_core/tests/test_image_normalize.py::DpiTest::test_300dpi_kaynak_72ye_iner_piksel_korunur`<br>`tradehub_core/tests/test_image_normalize.py::DpiTest::test_dpi_72_olarak_yazilir`<br>…+3 iz |
| **FR-030** | Sistem, çıktı DPI metadata'sını açıkça yazmalıdır (ekran medyası 72, belge 200); metadata'yı tüketici uygulamanın varsayılanına bırakmamalıdır. | F3 | YOK | `tradehub_core/tests/test_contracts.py::ImageContractTest::test_master_dpi_yazar_pikseli_degistirmez`<br>`tradehub_core/tests/test_e2e_scenarios.py`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo02DpiPikselDusurmez::test_3000_300dpi_2400_72dpiye_iner` | `tradehub_core/tests/test_e2e_scenarios.py`<br>`tradehub_core/tests/test_image_normalize.py`<br>`tradehub_core/tests/test_image_normalize.py::DpiTest::test_300dpi_kaynak_72ye_iner_piksel_korunur`<br>`tradehub_core/tests/test_image_normalize.py::DpiTest::test_dpi_72_olarak_yazilir`<br>…+3 iz |
...
| **FR-033** | Sistem, `master.min_long_edge` altında kalan dosyayı reddetmemeli, "under-spec" işaretleyip hangi profilleri üretemediğini söylemelidir. | F3 | YOK | `tradehub_core/tests/test_contracts.py::ImageContractTest::test_rendition_upscale_yapmaz_under_spec_isaretler` | — |
...
| **FR-035** | Sistem, her master'dan slot politikasında tanımlı türev merdivenini üretmelidir. | F3 | YOK — bir yükleme → bir dosya | `tradehub_core/tests/test_e2e_scenarios.py`<br>`tradehub_core/tests/test_render.py::RenderSozlesmesiTesti::test_merdiven_per_format_matrisin_tamami`<br>`tradehub_core/tests/test_render_regression.py::FixtureYapisiTesti::test_merdiven_basamak_sayisi` | `tradehub_core/tests/test_api_contracts.py`<br>`tradehub_core/tests/test_dedup.py::AlgisalHashTesti::test_yeniden_boyutlandirma_ve_sikistirma_hashi_korur`<br>`tradehub_core/tests/test_e2e_scenarios.py`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo12TurevSilinirYenidenUretilir::test_yeniden_uretim_ayni_baytlari_verir`<br>…+11 iz |
| **FR-036** | Sistem, iki komşu profil arasındaki oran 1,25'in altındaysa profilleri birleştirmelidir. | F2 ✅ | YOK | **KAPSANMIYOR** | — |
| **FR-037** | Sistem, kare kutuya giden profilleri (`w96`–`w768`) `pad` ile 1:1'e dolgulamalı; detay/zoom kaynağı profilleri (`w1280`, `w1920`) `contain`… | F3 | YOK | `tradehub_core/tests/test_render.py::GeometriTesti::test_contain_orani_korur_dolgu_yok`<br>`tradehub_core/tests/test_render.py::GeometriTesti::test_pad_hedef_orani_kurar` | `tradehub_core/tests/test_image_classify.py::KapiTest::test_skip_guard_ile_kapi_atlanir`<br>`tradehub_core/tests/test_image_classify.py::SozlesmeTest::test_to_dict_serilestirilebilir`<br>`tradehub_core/tests/test_image_lqip.py::AlfaTest::test_opak_gorselde_alfa_biti_yok`<br>`tradehub_core/tests/test_image_lqip.py::BoyutTest::test_base64_gomulebilir`<br>…+9 iz |
| **FR-038** | Sistem, logo türevlerini kayıpsız üretmelidir. | F3 | HATALI — `seller_media` yolundan geçen her logo q80 WebP'ye… | `tradehub_core/tests/test_quality_ssim.py::PolitikaHedefiTesti::test_logo_slotlari_kayipsiz_ister` | `tradehub_core/tests/test_image_lqip.py::AlfaTest::test_alfali_fixturelar_alfa_bayragi_tasir`<br>`tradehub_core/tests/test_image_normalize.py::RenkUzayiTest::test_alfa_dusurulmez`<br>`tradehub_core/tests/test_media_security_gate.py::KesiklikOlcumu::test_olculemez_bicim_ret_uretmez` |
| **FR-039** | Sistem, `master.strip_metadata.gps` alanını `true` uygulamalı; GPS koordinatlarını çıktıdan silmelidir (KVKK). | F3 | YOK — EXIF temizliği kod tabanında yok; avatar ucu engine'e… | `tradehub_core/tests/test_image_normalize.py::MetadataTest::test_gps_silinir` | `tradehub_core/tests/test_image_normalize.py::MetadataTest::test_exif_korunsa_bile_gps_ifd_kaldirilir`<br>`tradehub_core/tests/test_image_normalize.py::MetadataTest::test_gps_silinir`<br>`tradehub_core/tests/test_image_normalize.py::MetadataTest::test_inv04_eski_politika_gps_false_dese_de_koordinat_yayinlanmaz`<br>`tradehub_core/tests/test_media_mime_exif.py::ExifSozlesmesiTesti::test_kaynak_gps_kasada_gorulur_yayin_ciktisinda_silinir` |
| **FR-040** | Sistem, türev dosyalarını orijinalin yanında, aynı shard dizininde, ekli suffix ile saklamalıdır. | F3 | KISMEN — isimlendirme+shard VAR, türev YOK | `tradehub_core/tests/test_contracts.py::DeliveryContractTest::test_turev_anahtari_shardi_korur`<br>`tradehub_core/tests/test_e2e_scenarios.py`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo06KirpmaDegisinceUrlDegisir::test_turev_adresi_version_hash_tasir` | — |
...
| **FR-042** | Sistem, poster'ı satıcı yüklememişse otomatik üretmelidir. | F3+ | YOK — poster boşsa kutu siyah kalıyor (Ç7) | `tradehub_core/tests/test_contracts.py::DeliveryContractTest::test_video_manifesti_poster_zorunlu` | — |
| **FR-043** | Sistem, mağaza kartı/arama sonucu hover'ı için 6 saniyelik, sessiz, döngülü önizleme klibi üretmelidir. | F3+ | YOK | `tradehub_core/tests/test_contracts.py::VideoContractTest::test_onizleme_klibi_boyut_kapisini_dayatmaz` | — |
...
| **FR-048** | Sistem, yalnız iki kuralın RED üretmesine izin vermelidir: `nsfw_content` ve `extreme_blur`. Diğer 7 kural en fazla `warn` döner ve yayını asla… | F3+ | YOK (politika yazılı, uygulayan kod yok) | **KAPSANMIYOR** | `tradehub_core/tests/test_image_classify.py`<br>`tradehub_core/tests/test_policy_engine.py::IhlalSozlesmesiTesti::test_olculemeyen_kural_gecti_sayilmaz`<br>`tradehub_core/tests/test_quality_ssim.py::PolitikaHedefiTesti::test_sinif_tahmini_gecerli_deger_uretir` |
| **FR-049** | Sistem, birden çok kural tetiklendiğinde en yüksek aksiyonu uygulamalı, uyarıları birikimli listelemelidir. | F3+ | YOK | `node: admin-panel/frontend/src/lib/media/policy/__tests__/policyEngine.test.js :: birden çok ihlalde EN YÜKSEK aksiyon kazanır, uyarılar düşmez`<br>`tradehub_core/tests/test_contracts.py::PolicyContractTest::test_karar_birlesimi_en_yuksek_aksiyonu_alir` | — |
...
| **FR-060** | Sistem, her reddi `(kod, retryable)` çifti olarak döndürmeli; istemci karar verirken hata metnine değil koda bakmalıdır. | F1 ✅ | VAR (çekirdek) — korunacak | `node: admin-panel/frontend/src/lib/media/upload/__tests__/session.test.js :: politika reddi YENİDEN DENENMİYOR — aynı dosya aynı cevabı verir`<br>`node: admin-panel/frontend/src/utils/__tests__/uploadPolicy.test.js :: politika reddi yeniden denenmiyor, geçici hata deneniyor`<br>`tradehub_core/tests/test_contracts.py::PolicyContractTest::test_hata_yaniti_kod_ve_retryable_tasir` | — |
| **FR-061** | Sistem, kodlu ret sözleşmesini tüm yükleme uçlarında kullanmalıdır. | F3 | KISMEN | **KAPSANMIYOR** | — |
| **FR-062** | Sistem, her kullanıcı mesajında iki soruyu yanıtlamalıdır: (1) NEDEN reddedildi, (2) NASIL düzeltilir. | F2 ✅ | VAR (şema düzeyinde) | `panel-e2e: admin-panel/frontend/tests/e2e/media-upload-security.spec.ts :: S1 — 1000×1000 altı görsel reddedilir ve düzeltici yönlendirme gösterili…`<br>`tradehub_core/tests/test_e2e_scenarios.py`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo01YetersizCozunurluk::test_satici_ne_yapacagini_ogrenir` | — |
| **FR-063** | Sistem, ret mesajını sahaya özgü uygulanabilir bir çözümle bitirmelidir. | F2 ✅ | YOK | `tradehub_core/tests/test_e2e_scenarios.py`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo01YetersizCozunurluk::test_satici_ne_yapacagini_ogrenir` | — |
...
| **FR-066** | Sistem, ölçü verisi gelmediğinde sayı uydurmamalı, ayrı bir mesaj anahtarı kullanmalıdır. | F3 | YOK | `tradehub_core/tests/test_contracts.py::ImageContractTest::test_kalite_olculemiyorsa_sayi_uydurmaz` | — |
...
| **FR-076** | Sistem, tek dosya tavanını plan zincirine dâhil etmelidir: `min(plan, politika, frappe)`. | F3+ | YOK | `tradehub_core/tests/test_contracts.py::PolicyContractTest::test_efektif_tavan_kesisimdir` | — |
...
| **FR-097** | Sistem, bir dosyanın son referansı kalktıktan sonra silme adayı olması için bir bekleme (grace) süresi uygulamalıdır. | F3+ | YOK | `tradehub_core/tests/test_usage.py::DiskOksuzuTesti::test_esik_gunu_sinirda`<br>`tradehub_core/tests/test_usage.py::DiskOksuzuTesti::test_genc_dosya_korunur` | — |
| **FR-098** | Sistem, "frontend'de sabit yazılmış görsel" boşluğunu makinece okunabilir biçimde işaretlemelidir. | F3+ | YOK | **KAPSANMIYOR** | — |
| **FR-099** | Sistem, saklama sürelerini konfigüre edilebilir yapmalıdır. | F3 | YOK | `tradehub_core/tests/test_retention.py::TestPolitikaDogrulama::test_from_mapping_kismi_ayari_birlestirir` | — |
| **FR-100** | Sistem, `legal_hold` işaretli varlığı hiçbir politikayla silmemelidir. | F3 | YOK | `tradehub_core/tests/test_retention.py::TestLegalHold::test_tutulan_nesne_bloke_edilir`<br>`tradehub_core/tests/test_retention.py::TestLegalHold::test_tutulan_nesne_islak_kosumda_da_silinmez` | — |
...
| **FR-103** | Sistem, orijinal dosyayı varsayılan olarak süresiz saklamalıdır. | F1 ✅ (fiilen) | VAR (fiilen) | `tradehub_core/tests/test_retention.py::TestKuruKosum::test_varsayilan_politika_hicbir_seyi_silmez` | — |
...
| **FR-105** | Sistem, uygulanmamış bir depolama hedefi seçildiğinde politikayı reddetmelidir. | F3+ | YOK | `tradehub_core/tests/test_media_mirror_runtime.py`<br>`tradehub_core/tests/test_retention.py::TestPolitikaDogrulama::test_bilinmeyen_hedef_reddedilir` | — |
| **FR-106** | Sistem, yedek alma ile temizlik sırasını korumalı: ÖNCE yedek, SONRA eskileri temizle. | F1 ✅ | VAR — korunacak | `tradehub_core/tests/test_media_mirror_runtime.py` | — |
| **FR-107** | Sistem, disk ile veritabanı arasındaki yetim/kırık kayıtları düzenli olarak uzlaştırmalıdır. | F3+ | KISMEN | `tradehub_core/tests/test_media_mirror_runtime.py`<br>`tradehub_core/tests/test_usage.py::DiskOksuzuTesti::test_kayitsiz_ve_eski_dosya_oksuz`<br>`tradehub_core/tests/test_usage.py::KayitOksuzuTesti::test_bagi_olmayan_eski_varlik_oksuz` | — |
...
| **FR-113** | Sistem, imzalı URL TTL'ini clamp etmelidir. | F1 ✅ | VAR — korunacak | `tradehub_core/tests/test_storage_adapters.py::StorageContractMixin::test_url_for_private_ttl_kelepcelenir`<br>`tradehub_core/tests/test_storage_adapters_minio.py::PresignTtlUyarlamasi::test_url_for_private_ttl_kelepcelenir` | — |
...
| **FR-115** | Sistem, satıcı izolasyonunu beş katmanda uygulamalı ve başka mağazanın dosyasının varlığını bile ifşa etmemelidir. | F1 ✅ | VAR — korunacak | `tradehub_core/tests/test_api_contracts.py::TeslimTesti::test_baska_magaza_taslagi_goremez`<br>`tradehub_core/tests/test_media_seller_backup.py::TestIzolasyon::test_baska_magazanin_yedegi_okunamaz` | — |
...
| **FR-119** | Sistem, SVG kabulünü on maddenin tamamı karşılanmadan açmamalıdır. | F3+ | VAR (reddediyor) — açılış koşulları YOK | `tradehub_core/tests/test_policy_engine.py::KuralDavranisiTesti::test_kosullu_svg_kapali_oldugu_icin_reddedilir` | `tradehub_core/tests/test_image_probe.py`<br>`tradehub_core/tests/test_media_security_gate.py`<br>`tradehub_core/tests/test_policy_engine.py::KuralDavranisiTesti::test_kosullu_svg_kapali_oldugu_icin_reddedilir`<br>`tradehub_core/tests/test_svg_sanitize.py` |
| **FR-120** | Sistem, SVG bayt tavanını birincil, düğüm tavanını ikincil savunma olarak uygulamalıdır. | F3+ | YOK | **KAPSANMIYOR** | — |
| **FR-121** | Sistem, görseli `srcset`/`sizes`/`<picture>` ile cihaza uygun boyda servis etmelidir. | F3+ | YOK | `e2e: tradehubfront/tests/e2e/media-delivery-manifest.spec.ts :: S2 vitrin yarısı — 2400×2400 türev içsel ölçüsüyle ve tavana kadar srcse…`<br>`node: admin-panel/frontend/src/components/media/delivery/__tests__/mediaDelivery.test.js :: `sizes` her `srcset` ile birlikte basılıyor`<br>`node: admin-panel/frontend/src/components/media/delivery/__tests__/mediaDelivery.test.js :: her `srcset` genişliğe göre artan ve tekilleştirilmiş`<br>`node: admin-panel/frontend/src/components/media/delivery/__tests__/mediaDelivery.test.js :: kaynaklar AVIF → WebP sırasıyla basılıyor, JPEG `<img>`'de kalıyor`<br>…+8 test | — |
...
| **FR-123** | Sistem, bugünkü aşırı-servis oranını azaltmalıdır. | F3+ | YOK | `e2e: tradehubfront/tests/e2e/media-delivery-manifest.spec.ts :: S3 vitrin yarısı — piksel tavanı uygulanmış türev bildirildiğinde ürün s…`<br>`tradehub_core/tests/test_contracts.py::DeliveryContractTest::test_overshoot_olculur` | — |
| **FR-124** | Sistem, CLS'i önlemek için her görsel kutusunda oran veya sabit ölçü rezerve etmelidir. | F3+ | KISMEN | `e2e: tradehubfront/tests/e2e/media-delivery-manifest.spec.ts :: S2 vitrin yarısı — 2400×2400 türev içsel ölçüsüyle ve tavana kadar srcse…`<br>`node: admin-panel/frontend/src/components/media/__tests__/mediaCls.test.js :: kaynak yokken kutu yine ayrılır — boş satır zıplamasın`<br>`node: admin-panel/frontend/src/components/media/__tests__/mediaCls.test.js :: medya ekranlarındaki her görsel kutusu ÖNCEDEN ayrılmış`<br>`node: admin-panel/frontend/src/components/media/__tests__/mediaCls.test.js :: ölçüsü BİLİNMEYEN görselde de kutu ayrılır — yanlış oran, kaymayan orand…`<br>…+6 test | — |
| **FR-125** | Sistem, konuşma içeren kapak videosunda VTT altyazıyı zorunlu tutmalıdır. | F3+ | YOK | **KAPSANMIYOR** | — |
| **FR-126** | Sistem, `prefers-reduced-motion` altında otomatik hareketi iptal etmeli, kullanıcının kendi başlattığı oynatmayı kısıtlamamalıdır. | F3+ | YOK | `node: admin-panel/frontend/src/components/media/delivery/__tests__/mediaDelivery.test.js :: `prefers-reduced-motion` kararı CSS'te DEĞİL, JS'te veriliyor` | — |
| **FR-127** | Sistem, otomatik oynatmada `muted` + `playsinline` özniteliklerini istisnasız uygulamalıdır. | F3+ | KISMEN | `node: admin-panel/frontend/src/components/media/delivery/__tests__/mediaDelivery.test.js :: sessiz-döngü modu üç koşulu birden kuruyor` | — |
...
| **FR-134** | Sistem, süre kuralını ölçülebilir hâle geldikten sonra uygulamalıdır. | F3 | YOK | `tradehub_core/tests/test_contracts.py::PolicyContractTest::test_video_olculemezse_manuel_incelemeye_duser` | — |
...
| **FR-137** | Sistem, migration (geriye dönük standartlaştırma) işini canlı yükleme kuyruğundan ayrı ve düşük öncelikli çalıştırmalıdır. | F3+ | YOK | `tradehub_core/tests/test_e2e_scenarios.py` | — |
...
| **FR-139** | Sistem, migration'da durdurma kriteri taşımalıdır. | F3+ | KISMEN — geri alma altyapısı VAR, durdurma kriteri politika | `tradehub_core/tests/test_e2e_scenarios.py` | — |
...
| **FR-143** | Sistem, `accept.max_megapixels_hard` değerini bayt tavanından değil, çözme anındaki bellek bütçesinden türetmeli; eşik gerçek kütüphaneyi kesen bir… | F3 | YOK — kod tarafında hiç kontrol yok; politika değeri de… | `tradehub_core/tests/test_contracts.py::ImageContractTest::test_probe_megapiksel_bombasini_reddeder` | — |
...
| **FR-145** | Sistem, CMYK ve diğer sRGB dışı renk uzaylarını kabul yolunda sRGB'ye çevirmeli; çeviriyi yalnız optimizasyondan geçen dosyalara bırakmamalıdır. | F3 | KISMEN — `engine.optimize` çeviriyor; kapılardan dönen… | `tradehub_core/tests/test_contracts.py::ImageContractTest::test_master_cmyk_srgbye_cevirir` | — |
| **FR-146** | Sistem, alfa kanalı taşıyan bir master'ı alfasız bir biçime düşürmemelidir. | F3 | YOK — format seçiminde alfa kontrolü yok | `tradehub_core/tests/test_contracts.py::ImageContractTest::test_master_alfayi_dusurmez`<br>`tradehub_core/tests/test_image_normalize.py::RenkUzayiTest::test_alfa_dusurulmez` | — |
| **FR-147** | Sistem tek bir kaynak-doğru politika setine dayanmalıdır; iki set aynı anda yürürlükte olamaz. | F2 (karar) → F3 | YOK — karar verilmedi (T6) | `tradehub_core/tests/test_contracts.py::PolicyContractTest::test_source_root_raporlanir` | — |
| **FR-148** | Sistem'in politika doğrulama betiği her koşumda çalışabilir olmalı ve tek bir çıkış koduyla sonuç vermelidir. | F2 | HATALI — betik yazıldığı gibi koşamıyor | `tradehub_core/tests/test_contracts.py::PolicyContractTest::test_dogrulama_d1_d5` | — |
...
| NFR | Gereksinim (kısaltılmış) | Faz | SRS'teki bugünkü durum | Bağlayıcı test (A/C/F/I) | Fixture izi (B — sayılmaz) |
|---|---|---|---|---|---|
...
| **NFR-003** | Teslim edilen video rendition'ı dosya kapısını aşmamalıdır. | F3+ | YOK | `tradehub_core/tests/test_contracts.py::VideoContractTest::test_transcode_ses_silme_ve_dosya_kapisi` | — |
...
| **NFR-008** | Core Web Vitals bütçeleri karşılanmalıdır. | F3+ | ÖLÇÜLMEDİ | `tradehub_core/tests/test_e2e_scenarios.py::Senaryo03PikselTavani::test_urun_sayfasi_LCP_OLCULMEDI` | — |
...
| **NFR-011** | Optimizasyon kapıları korunmalıdır: gereksiz yeniden encode yapılmamalıdır. | F1 ✅ | VAR — korunacak | `tradehub_core/tests/test_render.py::DefterTesti::test_ikinci_kosum_encode_etmez`<br>`tradehub_core/tests/test_render.py::DefterTesti::test_merdiven_idempotent` | — |
...
| **NFR-020** | Yol geçişi (path traversal) iki bağımsız katmanda engellenmelidir. | F1 ✅ | VAR — korunacak | `tradehub_core/tests/test_media_access.py::DownloadTests::test_path_traversal_download_da_reddedilir`<br>`tradehub_core/tests/test_storage_adapters.py::TestLocalDiskDavranisi::test_yol_gecisi_reddedilir` | — |
| **NFR-021** | Sistem kendi kriptosunu yazmamalıdır. | F1 ✅ | VAR — korunacak | `tradehub_core/tests/test_storage_adapters.py::TestSignedUrl::test_default_signer_anahtarsiz_rastgele_uretmez`<br>`tradehub_core/tests/test_storage_adapters.py::TestSignedUrl::test_kurcalanan_imza_reddedilir` | — |
...
| **NFR-024** | İstemci sıkıştırmasının bilinçli kapatıldığı yerler geri açılmamalıdır. | F1 ✅ | VAR — korunacak | `vitest: tradehubfront/src/lib/upload-ui/uploader.test.ts :: compress:false verilirse prepareMedia hiç çağrılmaz, orijinal dosya ağa…` | — |
...
| **NFR-028** | Görsel yoksa arayüz kırılmamalıdır. | F1 ✅ | VAR — korunacak | `node: admin-panel/frontend/src/components/media/__tests__/mediaCls.test.js :: kaynak yokken kutu yine ayrılır — boş satır zıplamasın` | — |
...
| **NFR-033** | `alt` metni ve `loading`/`decoding` öznitelikleri uygulanmış kalmalıdır. | F3+ | KISMEN | `node: admin-panel/frontend/src/components/media/__tests__/mediaCls.test.js :: ölçüsü bilinen görsel intrinsic öznitelikleri basar`<br>`vitest: tradehubfront/src/components/media/ResponsiveImage.test.ts :: priority=false → loading=lazy, decoding=async` | — |
...
| **NFR-041** | Yarım dosya asla yazılmamalıdır. | F1 ✅ | VAR — korunacak | `tradehub_core/tests/test_e2e_scenarios.py`<br>`tradehub_core/tests/test_media_mirror_runtime.py`<br>`tradehub_core/tests/test_video_transcode.py::GercekTranscode::test_yarim_dosya_diskte_kalmaz` | — |
| **NFR-042** | Yedek/geri yükleme, transcode ile çatışmamalıdır. | F1 ✅ | VAR — korunacak | **KAPSANMIYOR** | — |
| **NFR-043** | `ffprobe` okunamadığında güvenli tarafa düşülmelidir. | F1 ✅ | VAR — korunacak | `tradehub_core/tests/test_contracts.py::PolicyContractTest::test_video_olculemezse_manuel_incelemeye_duser`<br>`tradehub_core/tests/test_contracts.py::VideoContractTest::test_probe_olculemedigini_soyler_hata_atmaz`<br>`tradehub_core/tests/test_retention.py::TestPolitikaBagimsizligi::test_kullanim_bilinmiyorsa_turev_korunur` | — |
| **NFR-044** | `file_url` sabit kalmalı; referanslar kırılmamalıdır. | F1 ✅ | VAR — korunacak (bir istisnayla) | `tradehub_core/tests/acceptance/test_storage_acceptance.py::TestKabul1LocalUctanUca::test_yukleme_teslim_uctan_uca`<br>`tradehub_core/tests/test_media_mirror_runtime.py`<br>`tradehub_core/tests/test_media_naming.py::TestWriteFileHashedFileDocPath::test_file_doc_gorunen_ad_degismez` | — |
...
| **NFR-050** | Aynı içerik tek fiziksel dosya olmalıdır. | F1 ✅ | VAR — korunacak | `panel-e2e: admin-panel/frontend/tests/e2e/media-dedup.spec.ts :: S9 — aynı dosya iki kez yüklendiğinde tek asset kalır (dedup)`<br>`tradehub_core/tests/test_e2e_scenarios.py`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo09AyniDosyaIkiKez::test_ikinci_yukleme_yeni_varlik_uretmez`<br>`tradehub_core/tests/test_media_mirror_runtime.py`<br>…+1 test | — |
| **NFR-051** | Dizin başına dosya sayısı ölçeklenebilir kalmalıdır. | F1 ✅ | VAR — korunacak | `tradehub_core/tests/test_dedup.py::AdlandirmaSozlesmesiTesti::test_ad_ve_shard_bicimi`<br>`tradehub_core/tests/test_e2e_scenarios.py`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo09AyniDosyaIkiKez::test_adres_icerikten_turer` | — |
| **NFR-052** | Tarih/saat gösterimi kullanıcının saat dilimine göre tek anlamlı olmalıdır. | F1 ✅ | VAR — korunacak | `node: admin-panel/frontend/src/utils/__tests__/dateFormat.test.js :: saat dilimi işareti olmayan tarih sunucu saati sayılıyor`<br>`node: admin-panel/frontend/src/utils/__tests__/dateFormat.test.js :: üç giriş biçimi de AYNI ana çözülüyor` | — |

### 2.3 Normatif invariant'lar (INV)

| INV | Kural | Faz | Durum | Doğrulanmış test (I) | Fixture izi (B — sayılmaz) |
|---|---|---|---|---|---|
| **INV-01** | Görsel asla büyütülmez (no upscale). | F6/F14 | Normatif | `tradehub_core/tests/golden/test_invariant_map.py`<br>`tradehub_core/tests/test_image_normalize.py::PikselTavaniTest::test_min_long_edge_kucuk_kaynagi_buyutmez`<br>`tradehub_core/tests/test_image_normalize.py::PikselTavaniTest::test_target_size_asla_buyutmez`<br>`tradehub_core/tests/test_render.py::RenderSozlesmesiTesti::test_merdiven_yetersiz_kaynaktan_basamak_uretmez` | — |
| **INV-02** | DPI metadata değişikliği piksel boyutunu değiştirmez. | F6/F14 | Normatif | `tradehub_core/tests/test_e2e_scenarios.py::Senaryo02DpiPikselDusurmez::test_3000_300dpi_2400_72dpiye_iner`<br>`tradehub_core/tests/test_image_normalize.py::DpiTest::test_300dpi_kaynak_72ye_iner_piksel_korunur`<br>`tradehub_core/tests/test_image_normalize.py::DpiTest::test_kabul_3000x3000_300dpi_2400x2400_72dpi` | — |
| **INV-03** | Maksimum kenar ve megapiksel tavanları uygulanır. | F6/F14 | Normatif | `tradehub_core/tests/test_image_normalize.py::PikselTavaniTest::test_megapiksel_tavani_uygulanir` | — |
| **INV-04** | Yayınlanan görsel GPS/EXIF bilgisi taşımaz. | F6/F14 | Normatif | `tradehub_core/tests/test_image_normalize.py::MetadataTest::test_gps_silinir` | — |
| **INV-05** | Video çıktısı aynı çözünürlükteki girdiden büyükse yayınlanmaz. | F6/F14 | Normatif | `tradehub_core/tests/test_render.py`<br>`tradehub_core/tests/test_render.py::DefterTesti::test_passthrough_masteri_zehirlemez`<br>`tradehub_core/tests/test_render.py::FaydaKapisiTesti::test_hicbir_turev_kaynaktan_buyuk_degil`<br>`tradehub_core/tests/test_render_regression.py`<br>…+6 test | — |
| **INV-06** | Aynı girdi ve politika yeniden encode edilmez (idempotency). | F6/F14 | Normatif | `tradehub_core/tests/test_api_contracts.py::YuklemeTesti::test_yaris_kosulunda_tek_varlik`<br>`tradehub_core/tests/test_dedup.py`<br>`tradehub_core/tests/test_dedup.py::YarisKosuluTesti::test_paralel_iki_finalize_tek_kayit`<br>`tradehub_core/tests/test_media_original_hash.py::TestOrijinalHashKatman3::test_ayni_iceriginin_ikinci_yuklemesi_ikinci_varlik_acmaz`<br>…+1 test | — |
| **INV-07** | Alfa kanalı korunur. | F6/F14 | Normatif | `tradehub_core/tests/test_image_normalize.py::RenkUzayiTest::test_alfa_dusurulmez` | — |
| **INV-08** | Bir varlığın tüm rendition'ları atomik yayınlanır veya hiçbiri yayınlanmaz. | F6/F14 | Normatif | `tradehub_core/tests/test_media_version_promote.py`<br>`tradehub_core/tests/test_media_version_promote.py::PromoteTests::test_promote_atomik_hata_da_tam_geri_alinir` | — |
| **INV-09** | Aynı içerik aynı hash URL'yi, farklı içerik farklı URL'yi verir. | F6/F14 | Normatif | `tradehub_core/tests/test_dedup.py`<br>`tradehub_core/tests/test_dedup.py::RenditionAdresiTesti::test_ayni_girdi_ayni_url`<br>`tradehub_core/tests/test_dedup.py::RenditionAdresiTesti::test_version_degisince_url_degisir`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo06KirpmaDegisinceUrlDegisir::test_kirpma_degisince_version_hash_degisir`<br>…+2 test | `tradehub_core/tests/test_video_decision.py` |
| **INV-10** | Normalize crop intent kaynaktan ve ölçekten bağımsızdır. | F6/F14 | Normatif | `tradehub_core/tests/test_api_contracts.py::KirpmaTesti::test_piksel_koordinati_reddedilir`<br>`tradehub_core/tests/test_crop.py`<br>`tradehub_core/tests/test_crop.py::Inv10Testi::test_yari_boyutlu_kaynak_ayni_kadraji_verir`<br>`tradehub_core/tests/test_e2e_scenarios.py::Senaryo04KirpmaVeCihazOnizleme::test_odak_tum_profil_oranlarinda_pencere_uretir`<br>…+1 test | — |
| **INV-11** | Orijinal retention izni olmadan silinmez; türev politikasından bağımsızdır. | F6/F14 | Normatif | `tradehub_core/tests/golden/test_invariant_guards.py::RetentionAuditContractAAA::test_retention_delete_routes_through_audited_trash`<br>`tradehub_core/tests/test_retention.py::TestPolitikaBagimsizligi::test_turev_silinirken_orijinal_korunur` | — |
| **INV-12** | 1000×1000 altı ürün görseli yayınlanamaz. | F6/F14 | Normatif | `tradehub_core/tests/golden/test_invariant_map.py`<br>`tradehub_core/tests/test_policy_engine.py::SinirVakalariTesti::test_bound_short999_reddedilir` | — |

## 3. KAPSANMAYAN gereksinimler — tam liste

Aşağıdaki **115** gereksinimin bu depoda hiçbir testi yok.
Gizlenmedi, sayıldı:
