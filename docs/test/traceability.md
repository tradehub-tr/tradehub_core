...
| Taranan Python test dosyası | 155 |
| Taranan Python test fonksiyonu | 3532 |
| Taranan FE test dosyası | 169 (node 80 · vitest 58 · e2e 27 · panel-e2e 4) |
| Taranan FE testi (koşan) | 1275 (node 886 · vitest 277 · e2e 102 · panel-e2e 10) |
...
| **FR-005** | Sistem, bir politikayı yalnız `open_questions` boş VE hiçbir `encoder_quality` `null` değil iken `status: "active"` kabul etmelidir; `draft` politika… | F3 | YOK — kapı tanımlı, uygulayan kod yok | `tradehub_core/tests/test_api_contracts.py::YonetimTesti::test_matris_kalibre_edilmemis_kaliteyi_isaretler` | — |
...
| **FR-039** | Sistem, `master.strip_metadata.gps` alanını `true` uygulamalı; GPS koordinatlarını çıktıdan silmelidir (KVKK). | F3 | YOK — EXIF temizliği kod tabanında yok; avatar ucu engine'e… | `tradehub_core/tests/test_image_normalize.py::MetadataTest::test_gps_silinir` | `tradehub_core/tests/test_image_normalize.py::MetadataTest::test_gps_silinir` |
